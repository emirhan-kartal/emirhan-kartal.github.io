...
const extra_services = {
    "online-service": { name: "Online Service", price: { monthly: '1', yearly: '10' } },
    "larger-storage": { name: "Larger storage", price: { monthly: '1', yearly: '10' } },
    "customizable-profile": { name: "Customizable profile", price: { monthly: '1', yearly: '10' } }
};
...
function getDatas(order) {
    ...
    if (order === 0) {
        inputs.forEach((temp) => { data_to_send[temp.id] = temp.value })
    }
    if (order === 1) {
        inputs.forEach((temp) => {
    ...
    if (order === 2) {
        inputs.forEach((temp) => {
            if (temp.checked) {
                console.log(bill_to_pay.extras.includes(temp.id))
...
function finalPage() {
    ...
    console.log(bill_to_pay.duration)
    ...
    sub_plan_dom.textContent = sub_plan_name + " (" + bill_to_pay.duration + ")";
    sub_plan_price_dom.textContent = sub_plan_price + "$/" +
        bill_to_pay.duration.substring(0, 1);

    let service_container_length =
        service_containers === null ? 0 : service_containers.length;

    if (bill_to_pay.extras.length !== service_container_length) {

        bill_to_pay.extras.forEach(element => {

            const extra_services_to_add =
                generateExtraServiceDom(element, bill_to_pay.duration);
            extra_services_dom.appendChild(extra_services_to_add);
            total_pay += parseInt(extra_services[element].price[bill_to_pay.duration]);
        });
    }
    total_pay_dom.textContent = total_pay + "$/" + bill_to_pay.duration.substring(0, 1);
...
function generateExtraServiceDom(service_id, duration) {
    ...
    console.log(service_data.price[duration] + "=" + duration)
    const service_price = service_data.price[duration] + "$/" + duration.substring(0, 1);
...
function changeSubDuration() {
    ...
    const extraServicePriceTags = document.querySelectorAll(".extras .text-blue-400");
